...
# Remember the program uses nigerian time
...
def main():
	...
	if response == "Y":
		beg_datetime = (date.today()-timedelta(days=1)).strftime('%d.%m.%Y') + " 22:00:00"
		end_datetime = date.today().strftime('%d.%m.%Y') + " 21:59:59"
	else:
	...
	beg_seconds = (beg_obj - datetime.datetime(1970,1,1)).total_seconds() - 3600 # since nigeria is utc +1
	end_seconds = (end_obj - datetime.datetime(1970,1,1)).total_seconds() - 3600 # since nigeria is utc +1
